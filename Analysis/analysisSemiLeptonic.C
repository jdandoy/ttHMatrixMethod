void analysisSemiLeptonic()
{
  ...
  TString name_dirmc_output("/data/atlas0/data/MC/MC15/DAOD_TOPQ1/filt/HistFinal/SemiLeptonic/");
  ...
  for (int iiso = 0; iiso <= 0; iiso++) {
    ...
    if (useData) {
      ...
      dataset.Process("doAna/MySemiLeptonic.C+",iso);
      timer.Stop(); timer.Print(); timer.Reset();
      // save output
      gROOT->ProcessLine(".! mv Ana-SemiLeptonic.root "+name_dir_output+"user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.SemiLeptonic"+iso+version+".root");
    }    
	...
	timer.Stop(); timer.Print(); timer.Reset();
	// save output
	gROOT->ProcessLine(".! mv Ana-SemiLeptonic.root "+name_dirmc_output+*it+".SemiLeptonic"+iso+version+".root");
      }
    }
  }
}
