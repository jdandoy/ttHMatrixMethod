...
void MMEffSet::AddTriggerSplit(){
  if(_isRun1){
    if(_isEle){
      AddCustomSplit("e24:e60:e120");
      _ptTres = 61;
    }
    else{
      AddCustomSplit("mu20:mu50");
      _ptTres = 51;
    }
  }
  else{
    std::cout << " add trigger split " << std::endl;
    if(!_isEle){
      AddCustomSplit("mu20i:mu50:mu20");
      _ptTres = 51;
    }
  }
...
void MMEffSet::ReadEfficiency(MMEfficiency *eff,std::string aEff,std::string condition,bool isCorrection){
  ...
  //std::cout << " ReadEfficiency " << std::endl;
  if(_debug){
  ...
  //std::cout << " apres histoName = " << histoName << std::endl;
