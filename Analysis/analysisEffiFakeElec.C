void analysisEffiFakeElec()
{
  ...
  TString name_dirmc_output("/data/atlas0/data/MC/MC15/DAOD_TOPQ1/filt/HistFinal/EffiFakeElec/new2/");
  ...
  for (int iiso = 0; iiso <= 1; iiso++) {
    ...
    for (int itrigger = 0; itrigger<=0; itrigger++) { 
      ...
      if (itrigger==1) trigger = "_HLT_e24_lhmedium_L1EM20VH";
      if (itrigger==2) trigger = "_HLT_e60_lhmedium";
      if (itrigger==3) trigger = "_HLT_e120_lhloose";
      if (itrigger==4) trigger = "_HLT_e24_lhmedium_iloose_L1EM20VH";
      
      // loop on selection
      for (int i = 5; i <= 9; i++) { 
	
	// CR (no cut)
	if (i==0)  option = "_lowETmissnocut";
	
	// CR HighMET (for real efficiencies)
	if (i==1)  option = "_highETmiss_pretag";
	if (i==2)  option = "_highETmiss_nobtag";
	if (i==3)  option = "_highETmiss_btag";
	
	// CR with MET+MT><60 GeV & MTW<20 GeV
	if (i==4)  option = "_lowETmissMTW_pretag";
	if (i==5)  option = "_lowETmissMTW_nobtag";
	if (i==6)  option = "_lowETmissMTW_btag";
	
	// CR with MET<20 GeV
	if (i==7)  option = "_lowETmiss20_pretag";
	if (i==8)  option = "_lowETmiss20_nobtag";
	if (i==9)  option = "_lowETmiss20_btag";
	
	// CR with MET<20 GeV & MT><20 GeV
	if (i==10)  option = "_lowETmissMTW20_pretag";
	if (i==11)  option = "_lowETmissMTW20_nobtag";
	if (i==12)  option = "_lowETmissMTW20_btag";
	
	// CR MET/sqrt(HT)<1
	if (i==13)  option = "_lowETmissHT1_pretag";
	if (i==14)  option = "_lowETmissHT1_nobtag";
	if (i==15)  option = "_lowETmissHT1_btag";
	
	// CR MET/sqrt(HT)<2
	if (i==16)  option = "_lowETmissHT2_pretag";
	if (i==17)  option = "_lowETmissHT2_nobtag";
	if (i==18)  option = "_lowETmissHT2_btag";
	
	// CR MET/sqrt(HT)<4
	if (i==19)  option = "_lowETmissHT4_pretag";
	if (i==20)  option = "_lowETmissHT4_nobtag";
	if (i==21)  option = "_lowETmissHT4_btag";
	...
	if (useData) {
	  ...
	  dataset.Process("doAna/MyEffiFakeElec.C+",option+iso+trigger);
	  timer.Stop(); timer.Print(); timer.Reset();
	  // save output
	  gROOT->ProcessLine(".! mv Ana-EffiFakeElec.root "+name_dir_output+"user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.EffiFakeElec"+option+iso+trigger+version+".root");
	}    
	...
	if (useMC) {
	  ...
	  for (std::vector<TString>::iterator it = names.begin(); it < names.end(); it++) {
	    ...
	    timer.Stop(); timer.Print(); timer.Reset();
	    // save output
	    gROOT->ProcessLine(".! mv Ana-EffiFakeElec.root "+name_dirmc_output+*it+".EffiFakeElec"+option+iso+trigger+versionout+".root");
	  }
	}
      }
    }
