...
public:
  ...
  void openFiles()
  {
    ...
    // data
    name = "user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.SemiLeptonic"+m_iso+".2337-2.root";
    m_fdata = new TFile(m_name_dir_datainput+name,"read"); 
    
    // fakes estimate from data
    name =  "user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.FakeSemiLeptonic_estimateEffPar"+m_iso+".2337-2.root";
    m_ffakesdata[0] = new TFile(m_name_dir_datainput+name);

    name =  "user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.FakeSemiLeptonic_estimateCRreal"+m_iso+".2337-2.root";
    m_ffakesdata[1] = new TFile(m_name_dir_datainput+name);
    name =  "user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.FakeSemiLeptonic_estimateCRfake"+m_iso+".2337-2.root";
    m_ffakesdata[2] = new TFile(m_name_dir_datainput+name);
    name =  "user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.FakeSemiLeptonic_estimateMCup"+m_iso+".2337-2.root";
    m_ffakesdata[3] = new TFile(m_name_dir_datainput+name);
    name =  "user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.FakeSemiLeptonic_estimateMCdown"+m_iso+".2337-2.root";
    ...
    // ====================================================
    // tbbar
    name = "user.derue.mc15_13TeV.ttbar.410000.SemiLeptonic"+m_iso+".2337-2.root";
    m_fttbar = new TFile(m_name_dir_mcinput+name,"read");
    std::cout << " name = " << name << std::endl;

 
    // single top
    name = "user.derue.mc15_13TeV.singletop.SemiLeptonic"+m_iso+".2337-2.root";
    m_fst = new TFile(m_name_dir_mcinput+name,"read"); 
    
    // diboson samples
    name = "user.derue.mc15_13TeV.diboson.SemiLeptonic"+m_iso+".2337-2.root";
    m_fdiboson = new TFile(m_name_dir_mcinput+name,"read");
    
    // Z
    name = "user.derue.mc15_13TeV.PowhegPythia8EvtGen_Z.SemiLeptonic"+m_iso+".2337-2.root";
    //name = "user.derue.mc15_13TeV.Sherpa_Z.SemiLeptonic.2337-2.root";
    m_fz = new TFile(m_name_dir_mcinput+name,"read");

    // W
    name = "user.derue.mc15_13TeV.PowhegPythia8EvtGen_W.SemiLeptonic"+m_iso+".2337-2.root";
    //name = "user.derue.mc15_13TeV.Sherpa_W.SemiLeptonic.2337-2.root";
    //name = "user.derue.mc15_13TeV.361100.PowhegPythia8EvtGen_AZNLOCTEQ6L1_Wplusenu.merge.DAOD_TOPQ1.e3601_s2576_s2132_r6765_r6282_p2454.SemiLeptonic.2337-2.root";

    m_fw = new TFile(m_name_dir_mcinput+name,"read");
    std::cout << " w = " << m_name_dir_mcinput+name << std::endl;
  ...
  void readHists() 
  {
	...
	if (var==m_max) m_ffakesdata[i]->Close(); 
      ...
      std::cout << " httbar = " << m_httbar[var] << std::endl;
      if (var==m_max) m_fttbar->Close(); 
      
      // read single top
      m_hst[var] = GetHistogram(var, m_fst);
      m_hst[var]->Scale(lumi);  
      std::cout << " hst = " << m_hst[var] << std::endl;
      if (var==m_max) m_fst->Close();
      
      // read diboson
      m_hdiboson[var] = GetHistogram(var, m_fdiboson);
      m_hdiboson[var]->Scale(lumi);  
     std::cout << " hdiboson = " << m_hdiboson[var] << std::endl;
      if (var==m_max) m_fdiboson->Close();

      // read Z
      m_hz[var] = GetHistogram(var, m_fz);
      m_hz[var]->Scale(lumi);  
      std::cout << " hz = " << m_hz[var] << std::endl;
      if (var==m_max) m_fz->Close();

      // read W
      m_hw[var] = GetHistogram(var, m_fw);
      m_hw[var]->Scale(lumi);  
     std::cout << " hw = " << m_hw[var] << std::endl;
      if (var==m_max) m_fw->Close();	
    }
  }
  ...
  void addHists()
  {
    ...
    for (int var=m_min; var<=m_max; var++ ) {
      ...
      //m_hfakesdata[var]->SetFillColor(kMagenta+3);
      m_hfakesdata[0][var]->SetFillColor(619);
      if (m_hfakesdata[0][var]->GetEntries()>0)
	m_hsum_all[var]->Add(m_hfakesdata[0][var]);
      
      // ttbar
      //std::cout << " ttbar " << std::endl;
      //m_httbar[var]->SetLineColor(kWhite);
      m_httbar[var]->SetFillColor(kWhite);
      if (m_httbar[var]->GetEntries()>0) m_hsum_all[var]->Add(m_httbar[var]);
      //m_hsum_all[var]->Add(m_httbar[var]);
      //std::cout << " ttbar entries = " << m_httbar[var]->GetSumOfWeights() << " " << m_httbar[var]->GetEntries() << std::endl;
      
      /// single top
      //std::cout << " single top " << std::endl;
      m_hst[var]->SetLineColor(62);
      m_hst[var]->SetFillColor(62);
      if (m_hst[var]->GetEntries()>0) m_hsum_all[var]->Add(m_hst[var]);
      
      /// diboson
      //std::cout << " diboson " << std::endl;
      m_hdiboson[var]->SetLineColor(5);
      m_hdiboson[var]->SetFillColor(5);
      if (m_hdiboson[var]->GetEntries()>0) m_hsum_all[var]->Add(m_hdiboson[var]);
 
      /// Z
      //std::cout << " Z " << std::endl;
      m_hz[var]->SetLineColor(95);
      m_hz[var]->SetFillColor(95);
      if (m_hz[var]->GetEntries()>0) m_hsum_all[var]->Add(m_hz[var]);

      /// W
      //std::cout << " W " << std::endl;
      m_hw[var]->SetLineColor(92);
      m_hw[var]->SetFillColor(92);
      if (m_hw[var]->GetEntries()>0) m_hsum_all[var]->Add(m_hw[var]);
      ...
      /*m_hstack_all[var]->Add(m_hdiboson[var]);
      m_hstack_all[var]->Add(m_hz[var]);
      m_hstack_all[var]->Add(m_hw[var]);
      m_hstack_all[var]->Add(m_hst[var]);
      m_hstack_all[var]->Add(m_httbar[var]);
      m_hstack_all[var]->Add(m_hfakesdata[0][var]);*/
      m_hstack_all[var]->Add(m_hfakesdata[0][var]);
      m_hstack_all[var]->Add(m_hdiboson[var]);
      m_hstack_all[var]->Add(m_hz[var]);
      m_hstack_all[var]->Add(m_hw[var]);
  ...
  void plotHists()
  {
    ...
    for (int var = m_min; var <= m_max; var++ ) {
      ...
      // plot axis
      plotAxisForAll(var,1,m_c[var],m_graxis1,m_hdata[var],m_hsum_all[var]);
      
      // plot histograms
      plot(var,m_hdata[var],m_hstack_all[var],GiveAxisName(var).c_str());

      // initialise arrays
      InitArrays();
      
      // handle asymetric errors
      for (int ibin=1; ibin<=n; ibin++) {
	// get bin center for x-axis
	m_x[ibin]   = m_hsum_all[var]->GetBinCenter(ibin);
	// get bin content for y-axis
	m_y[ibin]   = m_hsum_all[var]->GetBinContent(ibin);
	//std::cout << " ibin = " << ibin << " " << x[ibin]
	//	  << " " << y[ibin] << " " << m_hdata[var]->GetBinContent(ibin) << std::endl;
	// get bin width to have uncertainty for x-axis
	m_exl[ibin] = m_hsum_all[var]->GetBinWidth(ibin)/2;
	m_exh[ibin] = m_hsum_all[var]->GetBinWidth(ibin)/2;

	// statistical uncertainty on data
	float errstat = m_hdata[var]->GetBinError(ibin);
	errstat = 0.;
	// statistical and systematic uncertainty on fakes
	float errfake  = ErrFakes(var,ibin);
	// statistical and systematic uncertainty on MC
	float errmc    = ErrMC(var,ibin);
	// uncertainty on luminosity
	float errlumi  = 0.09*m_y[ibin];
	// total uncertainty
	float err      = sqrt(errstat*errstat+errfake*errfake+errmc*errmc+
			      errlumi*errlumi);
	//std::cout << " ibin = " << ibin << " " << m_x[ibin] << " " << m_y[ibin] << " errstat = " << errstat << " " << errfake << " " << errmc << " " << errlumi << " " << err << std::endl;
	m_eyh[ibin] = err;
	m_eyl[ibin] = err;
      }

      TGraphAsymmErrors *gr = 
	new TGraphAsymmErrors(n+1,m_x,m_y,m_exl,m_exh,m_eyl,m_eyh);
      ...
      // plot axis
      plotAxisForAll(var,2,m_c[var],m_graxis2,m_hdata[var],m_hsum_all[var]);
	...
	m_exl[ibin] = m_hsum_all[var]->GetBinWidth(ibin)/2;
	m_exh[ibin] = m_hsum_all[var]->GetBinWidth(ibin)/2;
	// calculate points only if two components have entries
	m_y[ibin]  = -100;
	m_y2[ibin] = -100;
	if (m_hdata[var]->GetBinContent(ibin)>0. && 
	    m_hsum_all[var]->GetBinContent(ibin)>0.) {
	  m_y[ibin] = m_hdata[var]->GetBinContent(ibin)/m_hsum_all[var]->GetBinContent(ibin);
	  m_y2[ibin] = 1.;
	}

	// statistical uncertainty on data
	float errstatdata = m_hdata[var]->GetBinError(ibin);
	// statistical and systematic uncertainty on fakes
	float errfake  = ErrFakes(var,ibin);
	// statistical and systematic uncertainty on MC
	float errmc    = ErrMC(var,ibin);
	...
	float errlumi  = 0.09*m_y[ibin];
	// total uncertainty 
	//float err      = sqrt(errstatdata*errstatdata+
	//		      errfake*errfake+errmc*errmc+
	//		      errlumi*errlumi);
	float err      = sqrt(errfake*errfake+errmc*errmc+
			      errlumi*errlumi);
	
	m_eyh[ibin]  = errstatdata/m_hsum_all[var]->GetBinContent(ibin);
	m_eyl[ibin]  = errstatdata/m_hsum_all[var]->GetBinContent(ibin);
	m_eyh2[ibin] = err/m_hsum_all[var]->GetBinContent(ibin);
	m_eyl2[ibin] = err/m_hsum_all[var]->GetBinContent(ibin);
      }
       
      TGraphAsymmErrors *gr2 = 
	new TGraphAsymmErrors(n+1,m_x,m_y,m_exl,m_exh,m_eyl,m_eyh);
      gr2->SetMarkerStyle(21);
      gr2->SetFillStyle(3454);
      TGraphAsymmErrors *gr3 = 
	new TGraphAsymmErrors(n+1,m_x,m_y2,m_exl,m_exh,m_eyl2,m_eyh2);
      gr3->SetFillStyle(3454);

      std::cout << " plot the graph " << std::endl;
      // plot the graph
      if (var == 0) {    // ETmiss
	if (m_cuts.Contains("ge1j") || m_channel.Contains("EMu")) {
	  plot(gr2,GiveAxisName(var).c_str(),0.,250.);
	} else {
	  plot(gr2,GiveAxisName(var).c_str(),50.,250.);
	}
      } 
      else if (var == 1) {
	if (m_cuts.Contains("ge1j") || m_channel.Contains("EMu")) {
	  plot(gr2,GiveAxisName(var).c_str(),0.,900.);
	} else {
	  plot(gr2,GiveAxisName(var).c_str(),100.,900.);
	}
      }
      else 
	plot(gr2,GiveAxisName(var).c_str());
      ...
      // for njet axis
      /*if (var==11) {
	if (m_cuts.Contains("ge1j")) {	
	  myText(0.25,0.29,1,"1"); 
	  myText(0.25,0.29,1,"2"); 
	  myText(0.8,0.29,1,"4"); 
	}
	if (m_cuts.Contains("ge2j")) {	
	  myText(0.25,0.29,1,"2"); 
	  myText(0.8,0.29,1,"4"); 
	}
	}*/
  ...
  void SetPad(int var)
  {
    Float_t x3[2];
    Float_t y3[2]; 
    x3[0] = m_hsum_all[var]->GetBinCenter(1);
    x3[1] = m_hsum_all[var]->GetBinCenter(m_hsum_all[var]->GetNbinsX());
    y3[0] = 0.;
    y3[1] = 2.;
  ...
  // ==================================================================
  float ErrFakes(int var, int ibin) 
  {
    //
    // statistical and systematic uncertainty on fakes
    //
    ...
    // number of fakes
    float fake = m_hfakesdata[0][var]->GetBinContent(ibin);
    if (fake<=0.) return 0.;

    // statistical uncertainty
    float errstat = sqrt(fake);

    // breakdown of each systematic uncertainty
    float errCRreal = std::fabs(fake-m_hfakesdata[1][var]->GetBinContent(ibin));
    float errCRfake = 0.;//std::fabs(fake-m_hfakesdata[2][var]->GetBinContent(ibin));
    float errMCup = std::fabs(fake-m_hfakesdata[3][var]->GetBinContent(ibin));
    float errMCdown = std::fabs(fake-m_hfakesdata[4][var]->GetBinContent(ibin));
    float errMC = 0.;
    if (errMCup>errMCdown) errMC = errMCup;
    else errMC = errMCdown;
    float errEffPar = std::fabs(fake-m_hfakesdata[5][var]->GetBinContent(ibin));
    // total systematic uncertainty
    float errsyst = sqrt(errCRreal*errCRreal+errCRfake*errCRfake+errMC*errMC+
			 errEffPar*errEffPar);
    // total uncertainty
    float err = sqrt(errstat*errstat+errsyst*errsyst);

    return err;
  }

  // ==================================================================
  float ErrMC(int var, int ibin) 
  {
    //
    ...
    // ttbar
    float errttbar = Errtttbar(var,ibin);
    //std::cout << " errttbar = " << errttbar << std::endl;

    // single top
    float errst = ErrSingleTop(var,ibin);
    //std::cout << " errst = " << errst << std::endl;

    // diboson
    float errdiboson = ErrDiboson(var,ibin);
    //std::cout << " errdiboson = " << errdiboson << std::endl;

    // Z boson
    float errz = ErrZJets(var,ibin);
    //std::cout << " errz = " << errz << std::endl;

    // W boson
    float errw = ErrWJets(var,ibin);
    //std::cout << " errw = " << errw << std::endl;

    float err = sqrt(errttbar*errttbar+errst*errst+errdiboson*errdiboson+
		     errz*errz+errw*errw);
    return err;
  }

  // ==================================================================
  float Errtttbar(int var, int ibin) 
  {
    //
    // ttbar
    //
    ...
    float x;
    float errx;
    x    = m_httbar[var]->GetBinContent(ibin);
    errx = m_httbar[var]->GetBinError(ibin);

    float err_xsec = 0.055*x;
    float err_stat = errx;
    float err_syst = CalcErrSyst(x);
    float err = sqrt(err_stat*err_stat+err_xsec*err_xsec+err_syst*err_syst);
    return err;
  }

  // ==================================================================
  float ErrSingleTop(int var, int ibin) 
  {
    //
    // single top
    //
    ...
    float x;
    float errx;
    x    = m_hst[var]->GetBinContent(ibin);
    errx = m_hst[var]->GetBinError(ibin);

    float err_xsec = 0.05*x;
    float err_stat = errx;
    float err_syst = CalcErrSyst(x);
    //std::cout << " x = " << x << " " << err_syst << std::endl;
    float err = sqrt(err_stat*err_stat+err_xsec*err_xsec+err_syst*err_syst);
    return err;
  }

  // ==================================================================
  float ErrDiboson(int var, int ibin) 
  {
    //
    // diboson
    //
    ...
    float x;
    float errx;
    x    = m_hdiboson[var]->GetBinContent(ibin);
    errx = m_hdiboson[var]->GetBinError(ibin);

    float err_xsec = 0.34*x;
    float err_stat = errx;
    float err_syst = 0.;//CalcErrSyst(x);
    float err = sqrt(err_stat*err_stat+err_xsec*err_xsec+err_syst*err_syst);
    return err;
  }

  // ==================================================================
  float ErrZJets(int var, int ibin) 
  {
    //
    // Z+jets
    //
    ...
    float x;
    float errx;
    x    = m_hz[var]->GetBinContent(ibin);
    errx = m_hz[var]->GetBinError(ibin);
  ...
  // ==================================================================
  float ErrWJets(int var, int ibin) 
  {
    //
    // W+jets
    //
    ...
    float x;
    float errx;
    x    = m_hw[var]->GetBinContent(ibin);
    errx = m_hw[var]->GetBinError(ibin);
  ...
  void PrintDataFakes(int var) 
  {
    ...
    std::cout << " data entries     = " << m_hdata[var]->GetSumOfWeights() << " +/- " << sqrt(m_hdata[var]->GetSumOfWeights()) << std::endl;

    std::cout << " all entries      = " << m_hsum_all[var]->GetSumOfWeights() << std::endl;
  ...
  // ==================================================================
  void Printttbar(int var) 
  {
    //
    // ttbar
    //
    
    std::cout << " ttbar entries = ";
    float x = m_httbar[var]->GetSumOfWeights();
    
    float errstat    = sqrt(x);
    float errxsec    = 0.055*x;
    float errsyst    = CalcErrSyst(x);
    float errsysttot = sqrt(errxsec*errxsec+errsyst*errsyst);
    float errtot     = sqrt(errstat*errstat+errsysttot*errsysttot);
    ...
    std::cout << x 
	      ...
	      << std::endl;
    /*std::cout << x[0]
	      << " +/- " << 100.*errstat/x[0] << " %(stat) " 
	      << " +/- " << 100.*errxsec/x[0] << " %(xsec) " 
	      << " +/- " << 100.*errsyst/x[0] << " %(syst) " 
	      << " +/- " << 100.*errsysttot/x[0] << " %(syst_tot) " 
	      << " +/- " << 100.*errtot/x[0] << " %(tot) " 
	      << std::endl;*/
  }

  // ==================================================================
  void PrintSingleTop(int var) 
  {
    //
    // single top
    //

    std::cout << " SingleTop entries = ";
    float x = m_hst[var]->GetSumOfWeights();

    float errstat    = sqrt(x);
    float errxsec    = 0.05*x;
    float errsyst    = CalcErrSyst(x);
    float errsysttot = sqrt(errxsec*errxsec+errsyst*errsyst);
    float errtot     = sqrt(errstat*errstat+errsysttot*errsysttot);
  ...
  // ==================================================================
  void PrintDiboson(int var) 
  {
    //
    // diboson
    //

    std::cout << " Diboson entries = ";
    float x = m_hdiboson[var]->GetSumOfWeights();

    float errstat    = sqrt(x);
    float errxsec    = 0.34*x;
    float errsyst    = 0.;//CalcErrSyst(x);
    float errsysttot = sqrt(errxsec*errxsec+errsyst*errsyst);
    float errtot     = sqrt(errstat*errstat+errsysttot*errsysttot);
  ...
  // ==================================================================
  void PrintZjets(int var) 
  {
    //
    // Z+jets
    //

    std::cout << " Z+Jets entries = ";
    float x = m_hz[var]->GetSumOfWeights();
  ...
  // ==================================================================
  void PrintWjets(int var) 
  {
    //
    // W+jets
    //

    std::cout << " W+Jets entries = ";
    float x = m_hw[var]->GetSumOfWeights();
  ...
  void plotText(int var) {
   ...
   // ===================================================================
  void plotLegend(int var, TGraphAsymmErrors* gr) 
  {
    //
    // plot the TLegend
    //
    ...
    leg->SetTextFont(42);  
    leg->AddEntry(m_hdata[var],"Data 2015","lpe");
    leg->AddEntry(m_httbar[var],"t#bar{t}","f");
    leg->AddEntry(m_hst[var],"Single top","f");
    leg->AddEntry(m_hz[var],"Z + jets","f");
    leg->AddEntry(m_hw[var],"W + jets","f");
    leg->AddEntry(m_hdiboson[var],"Diboson","f");
    leg->AddEntry(m_hfakesdata[0][var],"NP & Fake Lep.","f");
  ...
  // ================================================
  void plot(int var, TH1F * h, THStack * s, TString title, 
	    float min = -9999., float max = -9999.)
  {
    ...
    if (m_log) h->GetYaxis()->SetRangeUser(10.,this->GetMaximum(h,s)+0.1*this->GetMaximum(h,s));
    else h->GetYaxis()->SetRangeUser(0.,this->GetMaximum(h,s)+0.5*this->GetMaximum(h,s));
    ...
    for (int i=1;i<=nbinx;i++) {
      std::cout << " i = " << i << " " << h->GetBinCenter(i) 
		<< " " << max //<< " " << h->GetBinContent(i) 
		<< " " << sum << std::endl;
      if (h->GetBinCenter(i)<max) continue;
      sum += h->GetBinContent(i);
      sums += s->GetBinContent(i);
    }
    std::cout << " ovfl = " << h->GetBinContent(nbinx+1) << std::endl;
    h->SetBinContent(30,sum+h->GetBinContent(nbinx+1));
    s->SetBinContent(30,sums);
    */
		      ...
		      TGraph* gr, TH1F *h1, TH1F *h2)
  {
    ...
    if (choice==1) {
      if (m_log) {
	y[0] = 1.;
	y[1] = this->GetMaximum(h1,h2)+scale*this->GetMaximum(h1,h2);
      } else {
	y[0] = 0.;
	y[1] = this->GetMaximum(h1,h2)+scale*this->GetMaximum(h1,h2);
      }
    ...
    if (choice==2) {
      gr->GetYaxis()->SetTitle("Data / Pred.");
...
private:
  // input files
  TFile *m_fdata, *m_ffakesdata[6], *m_fttbar, *m_fst, *m_fw, *m_fz, *m_fdiboson;
 
  // histograms
  TH1F *m_hdata[40], *m_hfakesdata[6][40], *m_httbar[40], *m_hst[40], *m_hw[40], *m_hz[40], *m_hdiboson[40];
  THStack *m_hstack_all[40];/// the stack of all MC histos   
  TH1F *m_hsum_all[40];/// the sum of all histos with uncertainty from fakes estimate
