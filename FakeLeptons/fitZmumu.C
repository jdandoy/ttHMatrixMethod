...
public:
  ...
  {
    //
    // Performs fitting of di-muon mass in Z mass range
    //
    ...
    TString leveljet = "";
    for (int ileveljet=0;ileveljet<=4;ileveljet++) {
      ...
      for (int ileveltrigger=1;ileveltrigger<=3;ileveltrigger++) {
	TString leveltrigger = "";
	if (ileveltrigger==0) leveltrigger = "";
	if (ileveltrigger==1) leveltrigger = "_HLT_mu20_iloose_L1MU15";
	if (ileveltrigger==2) leveltrigger = "_HLT_mu50";
	if (ileveltrigger==3) leveltrigger = "_HLT_mu20_L1MU15";
	...
	if (isTOPQ1) m_name_dir_input = "/data/atlas0/data/DATA/DATA15/DAOD_TOPQ1/filt/HistFinal/EffiFakeMuon/";
	else m_name_dir_input = "/data/atlas0/data/DATA/DATA15/DAOD_EXOT4/filt/HistFinal/EffiFakeMuon/";
	...
	name += period; 
	if (isTOPQ1)
	  name += ".physics_Main.DAOD_TOPQ1.p2452.EffiRealMuonTP";
	else 
	  name += ".physics_Main.DAOD_EXOT4.p2425.EffiRealMuonTP";
	...
	*/
	for (int ieff=8;ieff<=9;ieff++) {
	  ...
	  for (int bkgmodel=1;bkgmodel<=1;bkgmodel++) {
		...
		TString nameFile="eff-Zmumu-periodDtoJ";
		...
		else if (effbinpt)            nameFile+="_vsPt.txt";
	 ...
	 int nrebin, TString level1, TString level2) 
  {
    ...
    histoOS = (TH1F*) m_InputFileOS->GetDirectory("HistosEffiRealMuonTP")->Get(m_nameHistoOS);
    std::cout << " histoOS = " << histoOS << std::endl;
    histoSS = (TH1F*) m_InputFileOS->GetDirectory("HistosEffiRealMuonTP")->Get(m_nameHistoSS);
    ...
    RooRealVar *mee;
    mee = new  RooRealVar("mmumu","m_{#mu#mu}", xmin,xmax,"");
    ...
    //dataSS->plotOn(meeosframe, MarkerStyle(21),MarkerColor(2),LineColor(2), RooFit::Name("dataSS"),DataError(RooAbsData::SumW2));
    if (doosss) 
   ...
   dataSS->plotOn(meeosframe, MarkerStyle(21),MarkerColor(33),LineColor(33), RooFit::Name("dataSS"),DataError(RooAbsData::SumW2));
    ...
    meeosframe->SetXTitle("m_{#mu#mu} [GeV]");
    ...
    // myText(      0.55,0.73,1,"#intLdt=20.28 fb^{-1}",0.045);
		      ...
		      TString level1, TString level2)
  {
    ...
    m_nameHistoOS = "EffiRealMuonTP_Mmumu";
    ...
    if (effbinpt) {
      suffix = "_vsPt"; 
      if (ibin==1)  m_nameHistoOS += "_Pt_25to30GeV";
      if (ibin==2)  m_nameHistoOS += "_Pt_30to35GeV";
      if (ibin==3)  m_nameHistoOS += "_Pt_35to40GeV";
      if (ibin==4)  m_nameHistoOS += "_Pt_40to45GeV";
      if (ibin==5)  m_nameHistoOS += "_Pt_45to50GeV";
      if (ibin==6)  m_nameHistoOS += "_Pt_50to55GeV";
      if (ibin==7)  m_nameHistoOS += "_Pt_55to60GeV";
      if (ibin==8)  m_nameHistoOS += "_Pt_60to80GeV";
      if (ibin==9)  m_nameHistoOS += "_Pt_80to120GeV";
      if (ibin==10) m_nameHistoOS += "_Pt_gt120GeV";
    }

    // vs eta
    if (effbineta) {
      suffix = "_vsEta"; 
      if (ibin==1)  m_nameHistoOS  += "_Eta_0to025";
      if (ibin==2)  m_nameHistoOS  += "_Eta_025to050";
      if (ibin==3)  m_nameHistoOS  += "_Eta_050to075";
      if (ibin==4)  m_nameHistoOS  += "_Eta_075to100";
      if (ibin==5)  m_nameHistoOS  += "_Eta_100to125";
      if (ibin==6)  m_nameHistoOS  += "_Eta_125to150";
      if (ibin==7)  m_nameHistoOS  += "_Eta_150to175";
      if (ibin==8)  m_nameHistoOS  += "_Eta_175to200";
      if (ibin==9)  m_nameHistoOS  += "_Eta_200to225";
      if (ibin==10) m_nameHistoOS  += "_Eta_225to250";
    }
    ...
    if (effbinnbjet) {
      ...
      if (ibin==3) m_nameHistoOS += "_ge1bJet";
    }
		       ...
		       int fitrange,int sigmodel, int period)
  {
    ...
    if (effbineta) {
      suffix = "_vsEta"; 
      if (ibin==1)  m_titleOutOS  += "_Eta_0to025";
      if (ibin==2)  m_titleOutOS  += "_Eta_025to050";
      if (ibin==3)  m_titleOutOS  += "_Eta_050to075";
      if (ibin==4)  m_titleOutOS  += "_Eta_075to100";
      if (ibin==5)  m_titleOutOS  += "_Eta_100to125";
      if (ibin==6)  m_titleOutOS  += "_Eta_125to150";
      if (ibin==7)  m_titleOutOS  += "_Eta_150to175";
      if (ibin==8)  m_titleOutOS  += "_Eta_175to200";
      if (ibin==9)  m_titleOutOS  += "_Eta_200to225";
      if (ibin==10) m_titleOutOS  += "_Eta_225to250";
    }
   ...
   // vs MET
    ...
    if (effbinnbjet) {
      ...
      if (ibin==3) m_titleOutOS += "_ge1bJet";
    }
		       ...
		       FILE *pFile[3])
{
  ...
  for (int ibin=m_imin;ibin<=m_imax;ibin++) {
    ...
    if ((effbinmet || effbinmtw) && efftight>1.) {
      efftight = 1.;
      Errefftight = 0.;
    }
