...
void plotControlSemiLeptonicBkg()
{
...
public:
  ...
  void openFiles()
  {
    ...
    name =  "user.derue.Data-periodDtoJ.physics_Main.DAOD_TOPQ1.p2452.FakeSemiLeptonic_estimate"+m_iso+".2337-2.root";
  ...
  void addHists()
  {
    ...
    for (int var=m_min; var<=m_max; var++ ) {
      ...
      /// sum
      for (int i=0;i<=5;i++) 
	m_hsum_all[i][var] = GetHistogram(var);
      std::cout << " var = " << var << " " << m_hsum_all[0][var] << " " << m_hsum_all[0][var]->GetNbinsX() << std::endl;
      
      // fakes estimate
      m_hfakesdata[0][var]->SetLineColor(1);
      m_hsum_all[0][var]->SetLineColor(1);
      if (m_hfakesdata[0][var]->GetEntries()>0) 
	m_hsum_all[0][var]->Add(m_hfakesdata[0][var]);
      // fakes estimate CRreal
      m_hfakesdata[1][var]->SetLineColor(2);
      m_hsum_all[1][var]->SetLineColor(2);
      if (m_hfakesdata[1][var]->GetEntries()>0) 
	m_hsum_all[1][var]->Add(m_hfakesdata[1][var]);
      // fakes estimate CRfake
      m_hfakesdata[2][var]->SetLineColor(3);
      m_hsum_all[2][var]->SetLineColor(3);
      if (m_hfakesdata[2][var]->GetEntries()>0) 
	m_hsum_all[2][var]->Add(m_hfakesdata[2][var]);
      // fakes estimate MCup
      m_hfakesdata[3][var]->SetLineColor(4);
      m_hsum_all[3][var]->SetLineColor(4);
       if (m_hfakesdata[3][var]->GetEntries()>0) 
	m_hsum_all[3][var]->Add(m_hfakesdata[3][var]);
     // fakes estimate MCdown
      m_hfakesdata[4][var]->SetLineColor(6);
      m_hsum_all[4][var]->SetLineColor(6);
      if (m_hfakesdata[4][var]->GetEntries()>0) 
	m_hsum_all[4][var]->Add(m_hfakesdata[4][var]);
      // fakes estimate EffPar
      m_hfakesdata[5][var]->SetLineColor(7);
      m_hsum_all[5][var]->SetLineColor(7);
      if (m_hfakesdata[5][var]->GetEntries()>0) 
	m_hsum_all[5][var]->Add(m_hfakesdata[5][var]);

      // ttbar
      //std::cout << " ttbar " << std::endl;
      m_httbar[var]->SetLineColor(5);
      m_httbar[var]->SetFillColor(5);
      if (m_httbar[var]->GetEntries()>0) {
	for (int i=0;i<=5;i++) m_hsum_all[i][var]->Add(m_httbar[var]);
      }
      //std::cout << " ttbar entries = " << m_httbar[var]->GetSumOfWeights() << " " << m_httbar[var]->GetEntries() << std::endl;
      
      /// single top
      //std::cout << " single top " << std::endl;
      m_hst[var]->SetLineColor(5);
      m_hst[var]->SetFillColor(5);
      if (m_hst[var]->GetEntries()>0) {
	for (int i=0;i<=5;i++) m_hsum_all[i][var]->Add(m_hst[var]);
      }
      /// diboson
      //std::cout << " diboson " << std::endl;
      m_hdiboson[var]->SetLineColor(5);
      m_hdiboson[var]->SetFillColor(5);
      if (m_hdiboson[var]->GetEntries()>0) {
	for (int i=0;i<=5;i++) m_hsum_all[i][var]->Add(m_hdiboson[var]);
      }
      /// Z
      //std::cout << " Z " << std::endl;
      m_hz[var]->SetLineColor(5);
      m_hz[var]->SetFillColor(5);
      if (m_hz[var]->GetEntries()>0) {
	for (int i=0;i<=5;i++) m_hsum_all[i][var]->Add(m_hz[var]);
      }

      /// W
      //std::cout << " W " << std::endl;
      m_hw[var]->SetLineColor(5);
      m_hw[var]->SetFillColor(5);
      if (m_hw[var]->GetEntries()>0) {
	for (int i=0;i<=5;i++) m_hsum_all[i][var]->Add(m_hw[var]);
      }

      // ratios
      std::cout << " all  = " << m_hsum_all[0][var]->GetSumOfWeights()
		<< " " << m_hsum_all[1][var]->GetSumOfWeights()
		<< " " << m_hsum_all[2][var]->GetSumOfWeights()
		<< " " << m_hsum_all[3][var]->GetSumOfWeights()
		<< " " << m_hsum_all[4][var]->GetSumOfWeights()
		<< " " << m_hsum_all[5][var]->GetSumOfWeights()
		<< std::endl;
      m_hfakesratio[1][var] = GetHistogram(var);
      m_hfakesratio[1][var]->SetLineColor(2);
      m_hfakesratio[1][var]->SetLineWidth(2);
      m_hfakesratio[1][var]->Divide(m_hsum_all[1][var],m_hsum_all[0][var]);
      //
      m_hfakesratio[2][var] = GetHistogram(var);
      m_hfakesratio[2][var]->SetLineColor(3);
      m_hfakesratio[2][var]->SetLineWidth(2);
      m_hfakesratio[2][var]->Divide(m_hsum_all[2][var],m_hsum_all[0][var]);
      //
      m_hfakesratio[3][var] = GetHistogram(var);
      m_hfakesratio[3][var]->SetLineColor(4);
      m_hfakesratio[3][var]->SetLineWidth(2);
      m_hfakesratio[3][var]->Divide(m_hsum_all[3][var],m_hsum_all[0][var]);
      //
      m_hfakesratio[4][var] = GetHistogram(var);
      m_hfakesratio[4][var]->SetLineColor(5);
      m_hfakesratio[4][var]->SetLineWidth(2);
      m_hfakesratio[4][var]->Divide(m_hsum_all[4][var],m_hsum_all[0][var]);
      //
      m_hfakesratio[5][var] = GetHistogram(var);
      m_hfakesratio[5][var]->SetLineColor(6);
      m_hfakesratio[5][var]->SetLineWidth(2);
      m_hfakesratio[5][var]->Divide(m_hsum_all[5][var],m_hsum_all[0][var]);
      
      for (int i=0;i<=5;i++) {
	m_hsum_all[i][var]->SetLineWidth(2);
  ...
  void plotHists()
  {
    ...
    for (int var = m_min; var <= m_max; var++ ) {
      ...
      int n = m_hsum_all[0][var]->GetNbinsX();
      ...
      // plot axis
      plotAxisForAll(var,1,m_c[var],m_graxis1,m_hdata[var],m_hsum_all[0][var]);
      
      // plot histograms
      plot(var,1,m_hdata[var],GiveAxisName(var).c_str());
      ...
      // plot axis
      plotAxisForAll(var,2,m_c[var],m_graxis2,m_hdata[var],m_hsum_all[0][var]);
  ...
  void SetPad(int var)
  {
    Float_t x3[2];
    Float_t y3[2]; 
    x3[0] = m_hsum_all[0][var]->GetBinCenter(1);
    x3[1] = m_hsum_all[0][var]->GetBinCenter(m_hsum_all[0][var]->GetNbinsX());
    y3[0] = 0.;
    y3[1] = 2.;
  ...
  void PrintDataFakes(int var) 
  {
    ...
    std::cout << " all entries      = " << m_hsum_all[0][var]->GetSumOfWeights() << std::endl;
  ...
  void plotLegend(int var) 
  {
    ...
    leg->AddEntry(m_httbar[var],"MC bkg","f");
	    ...
	    float min = -9999., float max = -9999.)
  {
      ...
      if (min>-9000 && max >-9000)
	{
      ...
      h->Draw("Psame");
      m_hstack_all[var]->Draw("HISTsame");
      m_hsum_all[1][var]->Draw("HISTsame");
      m_hsum_all[2][var]->Draw("HISTsame");
      m_hsum_all[3][var]->Draw("HISTsame");
      m_hsum_all[4][var]->Draw("HISTsame");
      m_hsum_all[5][var]->Draw("HISTsame");
      m_hsum_all[0][var]->Draw("HISTsame");
      h->Draw("Psame");
    } else {
      m_hfakesratio[1][var]->GetXaxis()->SetTitle(title);
      m_hfakesratio[1][var]->GetXaxis()->SetTitle("Syst / Std");
      
      if (min>-9000 && max >-9000) 
	m_hfakesratio[1][var]->GetXaxis()->SetRangeUser(min,max);
      
      //m_hfakesratio[1][var]->SetMaximum(20.);      
      //m_hfakesratio[0][var]->Draw("HISTsame");
...
private:
  ...
  THStack *m_hstack_mc[40], *m_hstack_all[40];/// the stack of all MC histos   
  TH1F *m_hsum_all[6][40];/// the sum of all histos with uncertainty from fakes estimate
