...
void EffiRealMuon()
{
  EffiRealMuon * myeffi = new EffiRealMuon();
	...
	for (int icr=0;icr<=0;icr++) {
	  ...
	  for (int isample=0;isample<=2;isample++) {
	    ...
	    for (int ijet=0;ijet<=5;ijet++) {
	      ...
	      name += period; 
	      if (isTOPQ1) name += ".physics_Main.DAOD_TOPQ1.p2452.EffiFakeMuon";
	      else name += ".physics_Main.DAOD_EXOT4.p2425.EffiFakeMuon";
	      ...
	      // all range
	      if (effall) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsAv";
		pFile[0] = fopen (name+".txt","w");		
	      }
	      
	      // vs ET
	      if (effbinpt) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsPt";
		pFile[1] = fopen (name+".txt","w");		
	      }
	      
	      // vs eta
	      if (effbineta) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsEta";
		pFile[2] = fopen (name+".txt","w");		
	      }
	      
	      // vs dRClosestJet
	      if (effbindr) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsdR";
		pFile[3] = fopen (name+".txt","w");		
	      }
	      
	      // vs PtLeadingJet
	      if (effbinptleadjet) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsPtLeadingJet";
		pFile[4] = fopen (name+".txt","w");		
	      }
	      
	      // vs dPhi
	      if (effbindphi) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsdPhi";
		pFile[5] = fopen (name+".txt","w");		
	      }
	      
	      // vs d0sig
	      if (effbind0sig) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsd0sig";
		pFile[6] = fopen (name+".txt","w");		
	      }

	      // vs MET
	      if (effbinmet) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsMET";
		pFile[7] = fopen (name+".txt","w");		
	      }
	      
	      // vs MTW
	      if (effbinmtw) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsMTW";
		pFile[8] = fopen (name+".txt","w");		
	      }

	      // vs njet
	      if (effbinnjet) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsNJet";
		pFile[9] = fopen (name+".txt","w");		
	      }

	      // vs nbjet
	      if (effbinnbjet) {
		name = "eff-RealMuon";  name += suffixout; name += "_vsNbJet";
		pFile[10] = fopen (name+".txt","w");		
	      }
