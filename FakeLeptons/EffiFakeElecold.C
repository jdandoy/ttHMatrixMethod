...
public:
  ...
  EffiFakeElec()
  {
    ...
    for (int imc=1;imc<=1;imc++) {
      ...
      for (int ileveltrigger=2;ileveltrigger<=2;ileveltrigger++) {
	...
	*/
	for (int ieff=0;ieff<=0;ieff++) {
	  ...
	  TString m_name_dirmc_input = "";
	  if (isTOPQ1) m_name_dirmc_input = "/data/atlas0/data/MC/MC15/DAOD_TOPQ1/filt/HistFinal/EffiFakeElec/";
	  else m_name_dirmc_input = "/data/atlas0/data/MC/MC15/DAOD_EXOT4/filt/HistFinal/EffiFakeElec/";
	  ...
	  for (int icr=1;icr<=1;icr++) {
	    ...
	    if (icr==2) cr    = "_lowETmissHT1";
	    if (icr==3) cr    = "_lowETmissHT2";
	    if (icr==4) cr    = "_lowETmissHT4";
	    
	    // loop on sample
	    for (int isample=1;isample<=1;isample++) {
		...
		suffixout = cr+sample+leveliso+leveltrigger+leveljet+version;
